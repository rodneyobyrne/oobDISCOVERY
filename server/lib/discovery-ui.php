...
function oobRenderAccountPage(string $title, string $eyebrow, string $lede, string $body, int $status = 200, string $headerActions = '', bool $privateHeader = false): never
{
    ...
    if (strcasecmp(trim($eyebrow), 'Private Discovery invitation') === 0) {
        ...
        $body = '<section class="invitation-purpose"><p><strong>The more we understand about the questions, concerns, experiences, and challenges an audience brings, the better we can connect what they need with the organization working to support them.</strong></p><p>Your perspective becomes part of a shared Discovery project. It will be considered alongside other team perspectives so recurring patterns can become a clearer picture of the audiences this organization is trying to reach and serve.</p><p>To get started, create your secure Discovery account below.</p></section>' . $body;
    }
