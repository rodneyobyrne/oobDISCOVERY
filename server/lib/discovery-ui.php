...
.shell:has(.invitation-experience){width:min(100% - 2rem,960px);margin:clamp(.8rem,3vw,2.2rem) auto;border-top-width:4px;padding:clamp(1.35rem,3.5vw,2.6rem);box-shadow:12px 12px 0 rgba(0,0,0,.055)}.shell:has(.invitation-experience) h1{max-width:48rem;font-size:clamp(2rem,4vw,3.2rem);line-height:1;letter-spacing:-.045em}.invitation-project-name{margin:.4rem 0 0;font-size:clamp(1.2rem,2.2vw,1.65rem);font-weight:800;line-height:1.15}.shell:has(.invitation-experience) .lede{max-width:48rem;margin:.55rem 0 0;font-size:1rem;line-height:1.5}.shell:has(.invitation-experience) .rule{height:2px;margin:1rem 0 1.05rem;transform:none}.invitation-purpose{max-width:49rem;display:grid;gap:.4rem;margin:0 0 .9rem}.invitation-purpose p{margin:0;color:#35312e;font-size:.94rem;line-height:1.5}.invitation-purpose strong{font-size:1rem}.invitation-experience{display:grid;gap:1rem}.primary-account-card{order:1;max-width:720px;padding:clamp(1.1rem,3vw,1.7rem);background:var(--warm);border:1px solid var(--warm-line)}.primary-account-card h2{margin:.15rem 0 .3rem;font-size:clamp(1.65rem,3vw,2.25rem);line-height:1.05;letter-spacing:-.04em}.primary-account-card .help{max-width:40rem;margin:.25rem 0 .6rem;font-size:.9rem;line-height:1.45;color:#45413c}.primary-account-card .form{gap:.4rem}.primary-account-card .form input{min-height:50px;background:#fff}.primary-account-card .form label{margin-top:.4rem}.primary-account-card .form button{margin-top:.7rem}.password-pair{display:grid;grid-template-columns:1fr 1fr;gap:1rem}.next-step{margin:.65rem 0 0;color:#4c4944;font-size:.82rem}.invitation-assurance{order:2;display:grid;grid-template-columns:repeat(3,1fr);gap:1px;background:var(--warm-line);border:1px solid var(--warm-line)}.assurance-item{background:var(--warm);padding:.9rem 1rem}.assurance-item strong{display:block;margin-bottom:.15rem;font-size:.86rem}.assurance-item span{display:block;color:#47433f;font-size:.8rem;line-height:1.4}.invitation-note{order:3;display:flex;align-items:flex-start;gap:.65rem;margin:0;padding:.75rem .9rem;background:#fff;border-left:3px solid var(--ink);font-size:.84rem}.invitation-note::before{content:"✓";font-weight:900}.existing-account{order:4;border-top:1px solid var(--line);border-bottom:1px solid var(--line);padding:.1rem 0}.existing-account summary{cursor:pointer;list-style:none;padding:.8rem 0;font-weight:800}.existing-account summary::-webkit-details-marker{display:none}.existing-account summary::after{content:"+";float:right;font-size:1.2rem}.existing-account[open] summary::after{content:"–"}.existing-account .existing-inner{max-width:720px;padding:0 0 1.1rem}.existing-account h2{margin:.15rem 0 .35rem;font-size:1.35rem}.existing-account .form{grid-template-columns:1fr 1fr auto;align-items:end;gap:.75rem}.existing-account .form .field{display:grid;gap:.3rem}.existing-account .form label{margin:0}.existing-account .form button{margin:0;min-height:48px}.existing-help{margin:.6rem 0 0;font-size:.84rem;color:var(--muted)}
@media(max-width:760px){.account-header-inner{align-items:flex-start;flex-direction:column;padding:1rem 0}.account-brand img{height:68px}.account-header-actions{width:100%;justify-content:flex-start}.split{grid-template-columns:1fr}.inline{align-items:stretch;flex-direction:column}.inline button{width:100%}.shell{box-shadow:none}.shell:has(.invitation-experience){margin:.65rem auto;padding:1.25rem}.invitation-assurance{grid-template-columns:1fr}.password-pair{grid-template-columns:1fr}.existing-account .form{grid-template-columns:1fr}.existing-account .form button{width:100%}}
CSS;
}
...
function oobRenderAccountPage(string $title, string $eyebrow, string $lede, string $body, int $status = 200, string $headerActions = '', bool $privateHeader = false): never
{
    ...
    $invitationProjectMarkup = '';
    if (strcasecmp(trim($eyebrow), 'Private Discovery invitation') === 0) {
        $projectPrefix = 'You’re invited to ';
        $projectLabel = str_starts_with($title, $projectPrefix) ? trim(substr($title, strlen($projectPrefix))) : '';
        $title = 'Customer perspective discovery';
        $lede = 'Discovery is a way for the people closest to the work to help us better understand the people they serve.';
        $headerActions = '<span class="header-context">Private invitation</span>';
        if ($projectLabel !== '') $invitationProjectMarkup = '<p class="invitation-project-name">' . oobEscape($projectLabel) . '</p>';
        $body = '<section class="invitation-purpose"><p><strong>The more we understand about the questions, concerns, experiences, and challenges an audience brings, the better we can connect what they need with the organization working to support them.</strong></p><p>Your perspective will be considered alongside other team perspectives so recurring patterns can become a clearer picture of the people this organization is trying to reach and serve.</p></section>' . $body;
    }
    ...
    echo '<main class="shell"><p class="eyebrow">' . oobEscape($eyebrow) . '</p><h1>' . oobEscape($title) . '</h1>' . $invitationProjectMarkup . '<p class="lede">' . oobEscape($lede) . '</p><div class="rule"></div>' . $body . '<p class="privacy">Private project access. Do not enter patient-identifying information, medical records, or other regulated personal data.</p></main></body></html>';
    exit;
}
