...
function responseTextBlocks(array $payload): array
{
    ...
    foreach ((array)($payload['patientPatterns'] ?? []) as $index => $pattern) {
        ...
        $title = trim((string)($pattern['title'] ?? '')) ?: 'Priority situation ' . ($index + 1);
        foreach (['helpSeekingMoment','outsideView','privateExperience','repeatedPattern','temporaryFunction','desiredChange','contactHesitation','serviceFit','referralBoundary'] as $key) {
...
<main class="page">
  ...
  <section class="observations" aria-labelledby="observations-heading">
    ...
    <ul>
      <li>You selected <strong><?= count((array)($services['offered'] ?? [])) ?></strong> services and <strong><?= count($audiences) ?></strong> priority situations.</li>
      <?php if ($audiences !== []): ?><li>Your selected priority situations were: <strong><?= responseEscape(implode(', ', array_values(array_filter(array_map(static fn($item) => is_array($item) ? (string)($item['title'] ?? '') : '', $audiences))))) ?></strong>.</li><?php endif; ?>
  ...
  <div id="source-response">
    <section class="source-section"><p class="eyebrow">Source response</p><h2>Services you selected</h2><dl><?php sourceList('Services offered', $services['offered'] ?? []); sourceList('People served', $services['recipients'] ?? []); sourceText('Anything else about services, availability, or limits', $services['boundaries'] ?? ''); ?></dl></section>

    <section class="source-section"><h2>Priority situations you selected</h2><?php if ($audiences === []): ?><p class="empty-answer">None selected</p><?php endif; ?><?php foreach ($audiences as $audience): if (!is_array($audience)) continue; ?><div class="situation"><h3><?= responseEscape((string)($audience['title'] ?? 'Priority situation')) ?></h3><p><?= nl2br(responseEscape((string)($audience['situation'] ?? ''))) ?></p></div><?php endforeach; ?></section>

    <section class="source-section"><h2>What you wrote about each situation</h2><?php if ($patterns === []): ?><p class="empty-answer">No detailed situation responses.</p><?php endif; ?><?php foreach ($patterns as $pattern): if (!is_array($pattern)) continue; ?><div class="situation"><h3><?= responseEscape((string)($pattern['title'] ?? 'Priority situation')) ?></h3><dl><?php sourceText('What brings them to look for help', $pattern['helpSeekingMoment'] ?? ''); sourceText('What other people may see', $pattern['outsideView'] ?? ''); sourceText('What may be happening privately', $pattern['privateExperience'] ?? ''); sourceText('Repeated pattern', $pattern['repeatedPattern'] ?? ''); sourceText('What the behavior may provide temporarily', $pattern['temporaryFunction'] ?? ''); sourceText('What they want to change', $pattern['desiredChange'] ?? ''); sourceText('What may make them hesitate to contact someone', $pattern['contactHesitation'] ?? ''); sourceText('Why Varetto may or may not fit', $pattern['serviceFit'] ?? ''); sourceList('Conditions you associated', $pattern['associatedConditions'] ?? []); sourceText('When you would refer elsewhere', $pattern['referralBoundary'] ?? ''); ?></dl></div><?php endforeach; ?></section>

    <section class="source-section"><h2>Website language answers</h2><dl><?php sourceText('What they might search or say', $narrative['searchLanguage'] ?? ''); sourceText('What they need to read to feel understood', $narrative['recognitionNeed'] ?? ''); sourceText('Language to avoid', $narrative['languageToAvoid'] ?? ''); sourceText('What Varetto can honestly promise about the experience', $narrative['honestPromise'] ?? ''); ?></dl></section>
  </div>
