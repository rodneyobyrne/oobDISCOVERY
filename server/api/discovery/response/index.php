...
try {
    if ($principal['system_admin']) {
        ...
        $statement = $pdo->prepare("SELECT s.* FROM discovery_submissions s JOIN discovery_user_clients uc ON uc.client_id = s.client_id WHERE s.submission_id = :submission_id AND s.discovery_type = 'clinician' AND uc.user_id = :user_id LIMIT 1");
        $statement->execute([':submission_id' => $submissionId, ':user_id' => (int)$principal['user_id']]);
    }
    ...
    oobRenderAccountPage('Response unavailable', 'Submitted response', 'This response is not available to your account or assigned projects.', '<div class="actions"><a class="button" href="/discovery/results/">Open results workspace</a></div>', 404);
}
...
$isOwner = !$principal['system_admin'] && (int)($row['owner_user_id'] ?? 0) === (int)($principal['user_id'] ?? 0);
?>
<!doctype html>
<html lang="en">
...
<main class="page">
  ...
  <h1>Here’s what was shared.</h1>
  <p class="lede">The original answers are kept intact below. Project members can review this source response; only the account that submitted it can edit it.</p>
  <div class="actions"><a class="button" href="#source-response">Review source response</a><?php if ($isOwner): ?><a class="button" href="https://discovery.oobcreative.com/<?= rawurlencode((string)$row['client_id']) ?>/?edit=<?= rawurlencode($submissionId) ?>">Edit my response</a><?php endif; ?><a class="button secondary" href="?submission_id=<?= rawurlencode($submissionId) ?>&amp;download=source">Download exact source</a><a class="button secondary" href="/discovery/project/?project_id=<?= rawurlencode((string)$row['client_id']) ?>">Project dashboard</a><a class="button secondary" href="/discovery/results/">Results workspace</a></div>
  ...
  <section class="observations" aria-labelledby="observations-heading">
    ...
    <ul>
      <?php if ($usesBaselineReviewV5): ?><li>This response opened <strong><?= count($audiences) ?></strong> persona <?= count($audiences) === 1 ? 'worksheet' : 'worksheets' ?> from the original study or a new therapy-audience hypothesis.</li><?php else: ?><li>This response selected <strong><?= count((array)($services['offered'] ?? [])) ?></strong> services and described <strong><?= count($audiences) ?></strong> <?= $usesAudienceMap ? 'candidate audience patterns' : 'priority situations' ?>.</li><?php endif; ?>
      <?php if ($audiences !== []): ?><li>The <?= $usesAudienceMap ? 'working audience labels' : 'selected priority situations' ?> were: <strong><?= responseEscape(implode(', ', array_values(array_filter(array_map(static fn($item) => is_array($item) ? (string)($item['title'] ?? '') : '', $audiences))))) ?></strong>.</li><?php endif; ?>
