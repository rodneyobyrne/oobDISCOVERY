...
$usesAudienceMapV4 = str_ends_with((string)($payload['questionnaireVersion'] ?? ''), '-v4');
...
<main class="page">
  ...
  <section class="observations" aria-labelledby="observations-heading">
    ...
    <ul>
      <li>You selected <strong><?= count((array)($services['offered'] ?? [])) ?></strong> services and described <strong><?= count($audiences) ?></strong> <?= $usesAudienceMapV4 ? 'candidate audience patterns' : 'priority situations' ?>.</li>
      <?php if ($audiences !== []): ?><li>Your <?= $usesAudienceMapV4 ? 'working audience labels' : 'selected priority situations' ?> were: <strong><?= responseEscape(implode(', ', array_values(array_filter(array_map(static fn($item) => is_array($item) ? (string)($item['title'] ?? '') : '', $audiences))))) ?></strong>.</li><?php endif; ?>
  ...
  <div id="source-response">
    <section class="source-section"><p class="eyebrow">Source response</p><h2>Practice reality</h2><dl><?php sourceList('Services offered', $services['offered'] ?? []); sourceList('Care or recovery contexts', $services['recipients'] ?? []); if ($usesAudienceMapV4) { sourceText('Populations already represented', $services['currentPopulation'] ?? ''); sourceText('Populations Varetto wants to serve more', $services['intendedPopulation'] ?? ''); } sourceText('Clinical, practical, or access boundaries', $services['boundaries'] ?? ''); if ($usesAudienceMapV4) sourceText('Honest promise about the experience of care', $narrative['honestPromise'] ?? ''); ?></dl></section>

    <section class="source-section"><h2><?= $usesAudienceMapV4 ? 'Candidate audience patterns' : 'Priority situations you selected' ?></h2><?php if ($audiences === []): ?><p class="empty-answer">None described</p><?php endif; ?><?php foreach ($audiences as $audience): if (!is_array($audience)) continue; ?><div class="situation"><h3><?= responseEscape((string)($audience['title'] ?? 'Audience pattern')) ?></h3><p><?= nl2br(responseEscape((string)($audience['situation'] ?? ''))) ?></p></div><?php endforeach; ?></section>

    <section class="source-section"><h2>What you wrote about each pattern</h2><?php if ($patterns === []): ?><p class="empty-answer">No detailed audience responses.</p><?php endif; ?><?php foreach ($patterns as $pattern): if (!is_array($pattern)) continue; ?><div class="situation"><h3><?= responseEscape((string)($pattern['workingLabel'] ?? $pattern['title'] ?? 'Audience pattern')) ?></h3><dl><?php if ($usesAudienceMapV4) { sourceText('Relationship to the intended practice', $pattern['audienceBasis'] ?? ''); sourceList('Sources informing the description', $pattern['evidenceSources'] ?? []); sourceText('Who the website is speaking to', $pattern['audienceRole'] ?? ''); sourceText('Relatively stable context', $pattern['stableContext'] ?? ''); sourceText('Help-seeking state or change', $pattern['helpSeekingState'] ?? ''); sourceText('Observable and private experience', $pattern['observedAndPrivate'] ?? ''); sourceText('Function and desired change', $pattern['functionAndDesiredChange'] ?? ''); sourceText('Resistance and trust', $pattern['resistanceAndTrust'] ?? ''); sourceText('Audience-specific language', $pattern['languageSignals'] ?? ''); sourceList('Clinical context', $pattern['clinicalContext'] ?? []); sourceText('Varetto fit and boundary', $pattern['fitAndBoundary'] ?? ''); sourceText('What should not carry over', $pattern['distinction'] ?? ''); } else { sourceText('What brings them to look for help', $pattern['helpSeekingMoment'] ?? ''); sourceText('What other people may see', $pattern['outsideView'] ?? ''); sourceText('What may be happening privately', $pattern['privateExperience'] ?? ''); sourceText('Repeated pattern', $pattern['repeatedPattern'] ?? ''); sourceText('What the behavior may provide temporarily', $pattern['temporaryFunction'] ?? ''); sourceText('What they want to change', $pattern['desiredChange'] ?? ''); sourceText('What may make them hesitate to contact someone', $pattern['contactHesitation'] ?? ''); sourceText('Why Varetto may or may not fit', $pattern['serviceFit'] ?? ''); sourceList('Conditions you associated', $pattern['associatedConditions'] ?? []); sourceText('When you would refer elsewhere', $pattern['referralBoundary'] ?? ''); } ?></dl></div><?php endforeach; ?></section>

    <?php if (!$usesAudienceMapV4): ?><section class="source-section"><h2>Website language answers</h2><dl><?php sourceText('What they might search or say', $narrative['searchLanguage'] ?? ''); sourceText('What they need to read to feel understood', $narrative['recognitionNeed'] ?? ''); sourceText('Language to avoid', $narrative['languageToAvoid'] ?? ''); sourceText('What Varetto can honestly promise about the experience', $narrative['honestPromise'] ?? ''); ?></dl></section><?php endif; ?>
  </div>
