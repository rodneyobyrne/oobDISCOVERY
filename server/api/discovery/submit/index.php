...
if ($_SERVER['REQUEST_METHOD'] === 'OPTIONS') {
    if ($origin === '' || !in_array($origin, $allowedOrigins, true)) {
        fail(403, 'Origin not allowed.', $requestId);
    }
...
if (!$isHttps) {
    fail(400, 'HTTPS is required.', $requestId);
}

$contentType = strtolower((string)($_SERVER['CONTENT_TYPE'] ?? ''));
if (strpos($contentType, 'application/json') !== 0) {
    fail(415, 'Content-Type must be application/json.', $requestId);
}

$maxBytes = (int)($config['max_payload_bytes'] ?? 262144);
$contentLength = (int)($_SERVER['CONTENT_LENGTH'] ?? 0);
if ($contentLength > $maxBytes) {
    fail(413, 'Request body is too large.', $requestId);
}

$raw = file_get_contents('php://input');
if ($raw === false || $raw === '') {
    fail(400, 'Request body is required.', $requestId);
}
...
if (!isAssocArray($payload)) {
    fail(400, 'Submission must be a JSON object.', $requestId);
}
...
if ($system !== 'oobDISCOVERY') {
    $errors[] = 'system is invalid.';
}
if ($discoveryType !== 'clinician') {
    $errors[] = 'discoveryType is invalid.';
}

$client = $payload['client'] ?? null;
if (!isAssocArray($client)) {
    $errors[] = 'client must be an object.';
    $client = [];
}
...
if (!isAssocArray($respondent)) {
    $errors[] = 'respondent must be an object.';
    $respondent = [];
}
...
if ($respondentEmail !== '' && filter_var($respondentEmail, FILTER_VALIDATE_EMAIL) === false) {
    $errors[] = 'respondent.email is invalid.';
}

$timing = $payload['timing'] ?? null;
if (!isAssocArray($timing)) {
    $errors[] = 'timing must be an object.';
} else {
    requireString($timing, 'startedAt', 64, $errors);
    requireString($timing, 'generatedAt', 64, $errors);
}
...
if (!is_array($archetypes)) {
    ...
    foreach ($archetypes as $i => $item) {
        if (!isAssocArray($item)) {
            $errors[] = 'archetypes[' . $i . '] must be an object.';
            continue;
        }
        foreach (['id', 'title', 'situation', 'relationship', 'caseload', 'websitePriority', 'note'] as $key) {
            if (!array_key_exists($key, $item) || !is_string($item[$key])) {
                $errors[] = 'archetypes[' . $i . '].' . $key . ' must be a string.';
            }
        }
        if (isset($item['relationship']) && !in_array($item['relationship'], $relationshipAllowed, true)) {
            $errors[] = 'archetypes[' . $i . '].relationship is invalid.';
        }
        if (isset($item['caseload']) && !in_array($item['caseload'], $caseloadAllowed, true)) {
            $errors[] = 'archetypes[' . $i . '].caseload is invalid.';
        }
        if (isset($item['websitePriority']) && !in_array($item['websitePriority'], $priorityAllowed, true)) {
            $errors[] = 'archetypes[' . $i . '].websitePriority is invalid.';
        }
    }
}

$patientPatterns = $payload['patientPatterns'] ?? null;
if (!is_array($patientPatterns)) {
    $errors[] = 'patientPatterns must be an array.';
} else {
    foreach ($patientPatterns as $i => $pattern) {
        if (!isAssocArray($pattern)) {
            $errors[] = 'patientPatterns[' . $i . '] must be an object.';
            continue;
        }
        foreach ($pattern as $key => $value) {
            if (!is_string($key) || !is_string($value)) {
                $errors[] = 'patientPatterns[' . $i . '] may contain string values only.';
                break;
            }
        }
    }
...
if (!isAssocArray($narrative)) {
    $errors[] = 'narrative must be an object.';
} else {
    foreach ($narrative as $key => $value) {
        if (!is_string($key) || !is_string($value)) {
            $errors[] = 'narrative may contain string values only.';
            break;
        }
    }
}
...
if ($errors !== []) {
    respond(422, [
        ...
        'details' => array_slice($errors, 0, 20),
        'requestId' => $requestId,
    ]);
}
...
try {
    $db = $config['database'] ?? [];
    $password = base64_decode((string)($db['password_b64'] ?? ''), true);
    if ($password === false) {
        throw new RuntimeException('Database password configuration is invalid.');
    }
    ...
    $statement = $pdo->prepare(
        ...
        . '(submission_id, discovery_type, client_id, respondent_name, respondent_email, questionnaire_version, payload_json) '
        . 'VALUES (:submission_id, :discovery_type, :client_id, :respondent_name, :respondent_email, :questionnaire_version, :payload_json)'
    );
    $statement->execute([
    ...
    if ((string)$e->getCode() === '23000') {
        fail(409, 'This submission has already been received.', $requestId);
    }
