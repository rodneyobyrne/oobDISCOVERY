...
function resultsCss(): string {
    return <<<'CSS'
:root{--ink:#111;--paper:#fff;--muted:#656565;--line:#c9c9c9;--soft:#f4f4f1;color-scheme:light;font-family:Arial,Helvetica,sans-serif}*{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);line-height:1.6}a{color:inherit;text-underline-offset:.18em}button,.button,input{font:inherit}.topbar{position:sticky;z-index:1000;top:0;background:#111;color:#fff}.topbar::after{content:"";position:absolute;right:0;bottom:-5px;left:0;height:10px;background:#fff;clip-path:polygon(0 34%,8% 27%,17% 42%,28% 30%,40% 42%,52% 29%,64% 42%,77% 29%,89% 41%,100% 34%,100% 67%,89% 60%,77% 71%,64% 61%,52% 71%,40% 61%,28% 72%,16% 60%,0 68%)}.topbar-inner,.page{width:min(1180px,calc(100% - 2rem));margin:auto}.topbar-inner{min-height:104px;display:flex;align-items:center;justify-content:space-between;gap:2rem}.workspace-brand{display:inline-flex;align-items:center;gap:1rem;color:#fff;text-decoration:none}.workspace-brand img{display:block;width:auto;height:82px;object-fit:contain;filter:invert(1)}.brand-copy{display:grid;line-height:1}.brand-name{font-size:1rem;font-weight:700;letter-spacing:-.04em}.brand-name span{color:#929292;font-weight:400}.brand-sub{margin-top:.45rem;font-size:.65rem;font-weight:700;letter-spacing:.14em;text-transform:uppercase}.top-actions,.card-actions{display:flex;align-items:center;flex-wrap:wrap;gap:.65rem}.account-identity{display:grid;justify-items:end;line-height:1.15;margin-right:.35rem}.account-name{font-size:.84rem;font-weight:700;color:#fff}.account-role{margin-top:.3rem;font-size:.66rem;font-weight:700;letter-spacing:.13em;text-transform:uppercase;color:#aaa}.button,button,.delete-control summary{display:inline-flex;align-items:center;justify-content:center;min-height:42px;padding:.65rem .9rem;border:1px solid #111;background:#111;color:#fff;text-decoration:none;font-weight:700;cursor:pointer}.topbar .button,.topbar button{border-color:#fff}.button.secondary{background:#fff;color:#111}.page{padding:clamp(2.5rem,6vw,5rem) 0 5rem}.eyebrow{margin:0 0 .6rem;font-size:.75rem;font-weight:800;letter-spacing:.13em;text-transform:uppercase}h1{margin:.2rem 0 1rem;font-size:clamp(2.8rem,6vw,5.5rem);line-height:.94;letter-spacing:-.055em}h2{margin:0;font-size:1.8rem;letter-spacing:-.035em}h3{margin:0;font-size:1.35rem;letter-spacing:-.025em}.lede{max-width:52rem;color:#333;font-size:1.08rem}.rule-note{margin:2rem 0;padding:1.15rem 1.25rem;border:1px solid #111;border-left:6px solid #111;background:var(--soft)}.response-count{margin:2.5rem 0 1rem;font-size:.78rem;font-weight:800;letter-spacing:.1em;text-transform:uppercase}.project-groups{display:grid;gap:3rem}.project-group{padding-top:1.2rem;border-top:5px solid #111}.project-heading{display:flex;align-items:flex-end;justify-content:space-between;gap:1.25rem;margin-bottom:1rem}.project-heading .meta{margin:.3rem 0 0}.response-list{display:grid;gap:.85rem}.response-card{display:grid;grid-template-columns:minmax(0,1fr) auto;align-items:center;gap:1.25rem;padding:1.25rem;border:1px solid #111}.response-card p{margin:.25rem 0 0}.meta{color:var(--muted);font-size:.87rem}.empty{padding:2rem;border:1px solid var(--line);background:var(--soft)}.login-shell{width:min(560px,calc(100% - 2rem));margin:8vh auto;padding:2rem;border:1px solid #111}.login-shell h1{font-size:clamp(2.3rem,6vw,4rem)}.login-form{display:grid;gap:.75rem}.login-form label{font-weight:700}.login-form input{width:100%;padding:.75rem;border:1px solid #999}.login-help{margin:.1rem 0 .35rem;color:var(--muted);font-size:.9rem}.notice{padding:1rem;border:1px solid #7a1e1e;color:#7a1e1e}.notice.success{border-color:#111;color:#111;background:var(--soft)}.delete-control{position:relative}.delete-control summary{list-style:none;background:#fff;color:#111}.delete-control summary::-webkit-details-marker{display:none}.delete-confirm{width:min(320px,80vw);margin-top:.5rem;padding:1rem;border:1px solid #111;background:#fff}.delete-confirm p{margin:0 0 .75rem;font-size:.86rem}.delete-confirm form{margin:0}.delete-confirm button{width:100%;background:#111;color:#fff}@media(max-width:760px){.topbar-inner{align-items:flex-start;flex-direction:column;padding:1rem 0}.workspace-brand img{height:68px}.top-actions{width:100%;justify-content:flex-start}.account-identity{justify-items:start;width:100%;margin-right:0}.project-heading{align-items:flex-start;flex-direction:column}.response-card{grid-template-columns:1fr}.card-actions,.card-actions .button,.card-actions .delete-control,.delete-control summary{width:100%}.delete-confirm{width:100%}}
CSS;
}
...
$resultsFlash = $_SESSION['results_flash'] ?? null;
unset($_SESSION['results_flash']);

if ($_SERVER['REQUEST_METHOD'] === 'POST' && (string)($_POST['action'] ?? '') === 'delete_response') {
    $returnUrl = '/discovery/results/';
    $returnUserId = filter_input(INPUT_POST, 'return_user_id', FILTER_VALIDATE_INT, ['options' => ['min_range' => 1]]);
    if ($returnUserId) $returnUrl .= '?user_id=' . (int)$returnUserId;

    $flashType = 'success';
    $flashMessage = '';
    try {
        if (!$isAdmin) throw new OobAuthException('Only Full Admins can delete responses.', 403, 'admin_required');
        if (!oobValidCsrf()) throw new InvalidArgumentException('Your session expired. Refresh the page and try again.');
        $submissionId = trim((string)($_POST['submission_id'] ?? ''));
        if ($submissionId === '' || strlen($submissionId) > 80 || preg_match('/[\x00-\x1F\x7F]/', $submissionId)) throw new InvalidArgumentException('Choose a valid response.');

        $lookup = $pdo->prepare("SELECT submission_id, respondent_name, client_id FROM discovery_submissions WHERE submission_id = :submission_id AND client_id <> 'deployment-check' LIMIT 1");
        $lookup->execute([':submission_id' => $submissionId]);
        $stored = $lookup->fetch();
        if (!$stored) throw new InvalidArgumentException('That response no longer exists.');

        $delete = $pdo->prepare("DELETE FROM discovery_submissions WHERE submission_id = :submission_id AND client_id <> 'deployment-check'");
        $delete->execute([':submission_id' => $submissionId]);
        $label = trim((string)$stored['respondent_name']) ?: 'Unnamed response';
        $flashMessage = 'Response deleted: ' . $label . '. This stored response has been permanently removed.';
    } catch (Throwable $error) {
        $flashType = 'error';
        $flashMessage = $error instanceof OobAuthException || $error instanceof InvalidArgumentException
            ? $error->getMessage()
            : 'The response could not be deleted.';
    }
    $_SESSION['results_flash'] = ['type' => $flashType, 'message' => $flashMessage];
    resultsRedirect($returnUrl);
}
...
try {
    if ($isAdmin) {
        ...
        if ($clientIds !== []) {
            ...
            $statement = $pdo->prepare("SELECT s.*, u.username AS owner_username, u.email AS owner_email, COALESCE(p.project_name, s.client_id) AS project_name, COALESCE(p.client_business_type, s.client_business_type, 'unclassified') AS business_type FROM discovery_submissions s LEFT JOIN discovery_users u ON u.id = s.owner_user_id LEFT JOIN discovery_projects p ON p.project_id = s.client_id WHERE s.client_id IN ({$placeholders}) ORDER BY COALESCE(p.project_name, s.client_id) ASC, s.updated_at DESC, s.id DESC LIMIT 250");
            $statement->execute($clientIds);
        } else {
            $statement = null;
        }
    }
    $submissions = $statement ? $statement->fetchAll() : [];
} catch (Throwable $error) {
...
foreach ($submissions as $row) {
    ...
    $groupedSubmissions[$projectId]['responses'][] = $row;
}
uasort($groupedSubmissions, static fn(array $a, array $b): int => strcasecmp((string)$a['project_name'], (string)$b['project_name']));

$currentUserId = (int)($principal['user_id'] ?? 0);
$title = $isAdmin ? ($filterUser ? 'User submissions' : 'Discovery responses') : 'Project responses';
$lede = $isAdmin
    ? ($filterUser
        ? 'Showing responses owned by ' . (string)$filterUser['username'] . ' (' . (string)$filterUser['email'] . '), organized by project.'
        : 'Responses are organized by project. Full Admins can view or permanently delete any stored Discovery response.')
    : 'Responses are organized by project. You can review every response inside projects assigned to your account, and you can edit only responses you personally submitted.';
...
<?php if (is_array($resultsFlash) && trim((string)($resultsFlash['message'] ?? '')) !== ''): ?><p class="notice <?= (string)($resultsFlash['type'] ?? '') === 'success' ? 'success' : '' ?>" role="status"><?= resultsEscape((string)$resultsFlash['message']) ?></p><?php endif; ?>
<div class="rule-note"><strong>Access rule:</strong> Full Admins can review every response. Clients can review every response in their assigned projects; only the original submitting account can edit a response. Response deletion is available only to Full Admins.</div>
<p class="response-count"><?= count($submissions) ?> response<?= count($submissions) === 1 ? '' : 's' ?> across <?= count($groupedSubmissions) ?> project<?= count($groupedSubmissions) === 1 ? '' : 's' ?></p>
<?php if ($submissions === []): ?><div class="empty"><?= $isAdmin && $filterUser ? 'This user has no owned submissions yet.' : 'No Discovery responses are available for these projects yet.' ?></div><?php else: ?><div class="project-groups">
<?php foreach ($groupedSubmissions as $group): ?><section class="project-group"><div class="project-heading"><div><p class="eyebrow">Project</p><h2><?= resultsEscape((string)$group['project_name']) ?></h2><p class="meta"><?= resultsEscape((string)$group['business_type']) ?> · <?= count($group['responses']) ?> response<?= count($group['responses']) === 1 ? '' : 's' ?></p></div></div><div class="response-list">
<?php foreach ($group['responses'] as $row): $name = trim((string)$row['respondent_name']) ?: 'Unnamed response'; $isOwner = !$isAdmin && (int)($row['owner_user_id'] ?? 0) === $currentUserId; ?>
<article class="response-card"><div><h3><?= resultsEscape($name) ?></h3><p class="meta"><?= resultsEscape(resultsFormatDate((string)($row['updated_at'] ?: $row['created_at']), $timezone)) ?></p><?php if ($row['respondent_email']): ?><p class="meta"><?= resultsEscape((string)$row['respondent_email']) ?></p><?php endif; ?><p class="meta">Owner: <?= $row['owner_username'] ? resultsEscape((string)$row['owner_username']) . ' · ' . resultsEscape((string)$row['owner_email']) : 'Unassigned historical submission' ?></p></div><div class="card-actions"><a class="button" href="/discovery/response/?submission_id=<?= rawurlencode((string)$row['submission_id']) ?>">View response</a><?php if ($isAdmin): ?><details class="delete-control"><summary>Delete response</summary><div class="delete-confirm"><p><strong>Permanent deletion.</strong> This removes the stored response from Discovery.</p><form method="post"><input type="hidden" name="csrf" value="<?= resultsEscape(oobCsrfToken()) ?>"><input type="hidden" name="action" value="delete_response"><input type="hidden" name="submission_id" value="<?= resultsEscape((string)$row['submission_id']) ?>"><?php if ($filterUser): ?><input type="hidden" name="return_user_id" value="<?= (int)$filterUser['id'] ?>"><?php endif; ?><button type="submit">Confirm delete</button></form></div></details><?php elseif ($isOwner): ?><a class="button secondary" href="https://discovery.oobcreative.com/<?= rawurlencode((string)$row['client_id']) ?>/?edit=<?= rawurlencode((string)$row['submission_id']) ?>">Edit my response</a><?php endif; ?></div></article>
<?php endforeach; ?></div></section><?php endforeach; ?></div><?php endif; ?>
</main></body></html>
